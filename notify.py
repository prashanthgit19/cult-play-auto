import os
import smtplib
from email.mime.text import MIMEText
from email.mime.multipart import MIMEMultipart


def send_notification(result, booked_class_info=None, config=None):
    if config is None:
        config = {}

    gmail_address = config.get("gmail_address", "") or os.environ.get("GMAIL_ADDRESS", "")
    gmail_app_password = (config.get("gmail_app_password", "") or os.environ.get("GMAIL_APP_PASSWORD", "")).replace(" ", "")
    notify_email = config.get("notify_email", "") or gmail_address
    user_name = config.get("name", "self")

    if not gmail_address or not gmail_app_password:
        print("Email credentials not configured. Skipping notification.")
        return

    if result == "success" and booked_class_info:
        subject = f"Cult.fit Play: Slot Booked Successfully! ({user_name})"
        body = (
            f"Your cult.fit Play slot has been booked!\n\n"
            f"Workout: {booked_class_info.get('workout_name', 'Unknown')}\n"
            f"Date: {booked_class_info.get('date', 'Unknown')}\n"
            f"Time: {booked_class_info.get('start_time', 'Unknown')}\n"
            f"Center ID: {booked_class_info.get('center_id', 'Unknown')}\n"
            f"Slot ID: {booked_class_info.get('slot_id', 'Unknown')}\n\n"
            f"Check your cult.fit app for details."
        )
    elif result == "waitlist" and booked_class_info:
        subject = f"Cult.fit Play: Slots Full - Joined Waitlist ({user_name})"
        body = (
            f"Slots were full. You have been added to the waitlist.\n\n"
            f"Workout: {booked_class_info.get('workout_name', 'Unknown')}\n"
            f"Date: {booked_class_info.get('date', 'Unknown')}\n"
            f"Time: {booked_class_info.get('start_time', 'Unknown')}\n"
            f"Center ID: {booked_class_info.get('center_id', 'Unknown')}\n"
            f"Slot ID: {booked_class_info.get('slot_id', 'Unknown')}\n\n"
            f"Check your cult.fit app for details."
        )
    elif result == "auth_expired":
        subject = f"Cult.fit: TOKEN EXPIRED - Action Required! ({user_name})"
        body = (
            f"Your cult.fit mobile app token (at) has expired for user '{user_name}'.\n\n"
            f"ACTION REQUIRED:\n"
            f"1. Open cult.fit app on iPhone\n"
            f"2. Set up mitmproxy and intercept traffic\n"
            f"3. Copy the new 'at' header value (CFAPP:...)\n"
            f"4. Update the Lambda env var for {user_name}\n\n"
            f"Tomorrow's booking will use the updated token."
        )
    else:
        subject = f"Cult.fit: Booking FAILED ({user_name})"
        body = (
            f"Failed to book a cult.fit class for user '{user_name}'.\n\n"
            f"All retry attempts were unsuccessful.\n"
            f"Check the CloudWatch logs for details."
        )

    msg = MIMEMultipart()
    msg["From"] = gmail_address
    msg["To"] = notify_email
    msg["Subject"] = subject
    msg.attach(MIMEText(body, "plain"))

    try:
        with smtplib.SMTP("smtp.gmail.com", 587) as server:
            server.starttls()
            server.login(gmail_address, gmail_app_password)
            server.sendmail(gmail_address, notify_email, msg.as_string())
        print(f"Notification email sent to {notify_email} for user '{user_name}'")
    except Exception as e:
        print(f"Failed to send notification email: {e}")